...
# iterates through the hash, checking all numerals 1 - 100
describe Romaniser do
  ...
  100.times do
    ...
    it { expect(subject.romanise(c)).to eq "#{hash_1_to_100[c.to_s]}" }
...
describe Romaniser do
  xit { expect(subject.romanise(99)).to eq "XCIX" }
  xit { expect(subject.romanise(629)).to eq "DCXXIX" }
end
